...
- Optional *bridged equality* via epoch-bound transform `t_{a→b}`.
...
- Show that bridging is a *controlled* transform and must be epoch-bound.
...
- Enabling **bridge** shows equivalence *under a resolver*, without revealing identity.
...
**Important:** UI should label these clearly as *deterministic placeholders*.
...
- “This demo simulates predicate attestations: the protocol checks *properties* with an issuer’s proof.”
